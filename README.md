...
\vspace*{\fill}
\LARGE План выполнения курсовой работы по теме: \\
"Платформа для соревнований по прогнозированию спортивных мероприятий"
\vspace*{\fill}
\end{titlepage}
...
Swagger - это независимая от языка спецификация для написания REST API. Она позволяет компьютерам и пользователям лучше понять функциональность REST API без необходимости прямого доступа к исходному коду, описывает структуру ответа и запроса.\\
Парсинг - это автоматический сбор и сохранение/организация данных, полученных с помощью кода, из открытых источников.\\
...
OAuth 2.0 - это протокол аутентификации для предоставления одному сервису/пользователю права доступа к ресурсам другого сервиса/пользователя.\\
Токены - это зачастую зашифрованные фрагменты данных, которые содержат ровно столько информации, сколько требуется для того, чтобы облегчить процесс определения личности пользователя или авторизации пользователя для выполнения каких-либо действий.\\
Deeplink - это гиперссылка, которая автоматически перенаправляет пользователя на определенный раздел приложения или веб-сайта.
...
\section{План работ}
01.12.2022 - 15.12.2022 - Изучение существующих приложений, предоставляющих подобный функционал\\
